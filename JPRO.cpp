...
enum TypFigury { TROJKAT, PROSTOKAT };
...
float pole(Figura figura) {
	...
	{
	case TROJKAT:
		return (figura.dane.trojkat.podstawa * figura.dane.trojkat.wysokosc) / 2;
		break;
	case PROSTOKAT:
		return figura.dane.prostokat.bok1 * figura.dane.prostokat.bok2;
		break;
	default:
		return 0;
		break;
...
int main()
{
	...
	Figura f;
	f.typ = TROJKAT;
	f.dane.trojkat.podstawa = 5;
	f.dane.trojkat.wysokosc = 10;

	std::cout << "Pole trojkata: " << pole(f) << std::endl;
